...
checkBrowsers(paths.appPath, isInteractive)
  ...
  .then(port => {
    ...
    let staticResourcesCopied = false;
    const copyStaticResources = () => {
      if(!staticResourcesCopied) {
        staticResourcesCopied = true;
        fs.copyFileSync(paths.extManifest, paths.appBuild + '/manifest.json')
        paths.staticsToCopy.forEach((item) => {
          const fPath  = paths.appBuild + item.to;
          copyRecursiveSync(item.path, fPath);
        })
      }
    }
